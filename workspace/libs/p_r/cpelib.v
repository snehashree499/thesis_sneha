 ...
 *
 * Revision history:
 *   2025-02-07 - Added CC_IO_SEL user clock input option.
 *   2025-02-07 - Fixed CC_IO_SEL input multiplexers.
 *   2024-12-18 - Add clock option from CPE to CC_IO_SEL.
...
module CC_IO_SEL #(
	...
	wire q2_i = IO_SEL_CFG[9]  ? OUT4_I : OUT1_I;
	wire q3_i = IO_SEL_CFG[10] ? OUT3_I : OUT2_I;

	wire usr_clkmux = IO_SEL_CFG[9] ? OUT1_I : OUT4_I;

	wire qoutmx0_o = IO_SEL_CFG[20] ? q2_o : q2_i;
	wire qoutmx1_o = IO_SEL_CFG[21] ? q3_o : q3_i;

	wire OMUX_SEL = (IO_SEL_CFG[13] & ~IO_SEL_CFG[12]) ? DDR_I : IO_SEL_CFG[11];
	assign GPIO_OUT = IO_SEL_CFG[8] ? (OMUX_SEL ? qoutmx1_o : qoutmx0_o) : (OMUX_SEL ? 1'b1 : 1'b0);

	wire clk_o = IO_SEL_CFG[15] ? usr_clkmux : (IO_SEL_CFG[25] ? (IO_SEL_CFG[24] ? CLOCK4 : CLOCK3) : (IO_SEL_CFG[24] ? CLOCK2 : CLOCK1));
	wire clk_i = IO_SEL_CFG[14] ? usr_clkmux : (IO_SEL_CFG[29] ? (IO_SEL_CFG[28] ? CLOCK4 : CLOCK3) : (IO_SEL_CFG[28] ? CLOCK2 : CLOCK1));

	wire q0clk = IO_SEL_CFG[30] ^ clk_i;
	wire q1clk = IO_SEL_CFG[31] ^ clk_i;
	wire q2clk = IO_SEL_CFG[26] ^ clk_o;
	wire q3clk = IO_SEL_CFG[27] ^ clk_o;

	assign IN1_O = IO_SEL_CFG[22] ? q0_o : GPIO_IN;
	assign IN2_O = IO_SEL_CFG[23] ? q1_o : GPIO_IN;
